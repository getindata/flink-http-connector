...
HTTP requests can fail, and can be retried. The usual behaviour after such a failure is to end the job. If you would like to 
handle there errors in your flow then you can set  the config option `gid.connector.http.source.lookup.fail-job-on-error` to false.
In this case the job will not fail and it is possible to pass the error information back. To allow this, 3 metadata columns have 
been added and will be populated with information from the error. 

These are all optional read-only columns that must be declared VIRTUAL to exclude them during an INSERT INTO operation.

| Key           | Data Type                        | Description                             |
|---------------|----------------------------------|-----------------------------------------|
| error_string  | STRING NULL                      | A message associated with the error     |
| error_code    | INT NULL                         | The HTTP status code if there is one    |
| error_headers | MAP <STRING, ARRAY<STRING>> NULL | The headers returned with the response_ |

If the above metadata columns are defined on the table and the call succeeds then they will have null values.
...
fail if `gid.connector.http.source.lookup.fail-job-on-error` is true, otherwise the error information is supplied in metadata columns.
...
| gid.connector.http.source.lookup.fail-job-on-error                                   | optional | When false, the error information is returned in the response in metadata columns. When true (the default) the job end on errors.                                                                                                                                                                                                                                                                                                                                                                                                                               |
