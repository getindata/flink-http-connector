...
processing after these failures then specify `continue_on_error` as true. THe lookup join will complete without content in the expected enrichment columns from the http call,
...
When using `continue_on_error` as true, consider adding extra metadata columns that will surface information about failures into your stream. In non failure situations these metadata
...
| continue_on_error_type | STRING NULL                      | When continue_on_error is set, this gives extra information about the cause of the error. |

##### continue_on_error_type possible values

| Value                      | Description                                         |
|:---------------------------|-----------------------------------------------------|
| ERROR_STRING               | Message associated with the error                   |
| HTTP_FAILED_AFTER_RETRY    | HTTP call failed after retrying                     |
| HTTP_FAILED                | HTTP request was not retried and failed             |
| CLIENT_SIDE_EXCEPTION      | No HTTP call completed and there was an Exception   |

If the error_string metadata column is defined on the table and the call succeeds then they it will have a null value.
...
| gid.connector.http.source.lookup.continue_on_error                                  | optional | When true, the error information is returned in the response in metadata columns. When false (the default) the job ends on errors.                                                                                                                                                                                                                                                                                                                                                                                                                               |
